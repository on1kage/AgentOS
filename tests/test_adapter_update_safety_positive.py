...
def test_adapter_update_safety_positive_scout():
    ...
    assert verify_adapter_output("scout", d["results"]["scout"], expected_action="external_research") is True

def test_adapter_update_safety_positive_envoy():
    d = json.loads(FIXTURE_PATH.read_text(encoding='utf-8'))
    assert verify_adapter_output("envoy", d["results"]["envoy"], expected_action="deterministic_local_execution") is True
