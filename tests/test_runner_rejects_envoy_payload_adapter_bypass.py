...
def test_runner_rejects_envoy_arbitrary_shell_payload_and_emits_no_run_started():
    with tempfile.TemporaryDirectory() as tmp:
        store = FSStore(root=tmp)
        task_id = "t_reject_envoy_bypass_1"
        ...
        store.append_event(
            ...
            "TASK_CREATED",
            {
                ...
                "payload": {
                    "exec_id": "exec-1",
                    "kind": "shell",
                    "cmd_argv": ["python3", "-c", "print('bypass')"],
                    "cwd": tmp,
                    "env_allowlist": [],
                    "timeout_s": 1,
        ...
        assert str(ei.value) == "adapter_binding_required:envoy:deterministic_local_execution"
